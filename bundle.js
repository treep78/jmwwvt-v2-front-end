...
/***/ function(module, exports, __webpack_require__) {
	...
	var portfolio = __webpack_require__(13);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var viewUi = __webpack_require__(12);
	var config = __webpack_require__(6);

	var onURLChange = function onURLChange(event) {
	  var newView = event.newURL.split('');
	  var oldView = event.oldURL.split('');
	  // Filter new view name
	  for (var i = 0; i < newView.length; i++) {
	    if (newView[i] === '#') {
	      var tempView = '';
	      for (var j = i; j < newView.length; j++) {
	        tempView += newView[j];
	        if (newView[j + 1] === '/') {
	          break;
	        }
	      ...
	      if (newView === '#portfolio') {
	        api.loadImages().then(ui.getImagesSuccess).then(viewUi.displayImages).catch(ui.failure);
	      }
	      break;
	    }
	  }
	  // Filter old view name
	  for (var _i = 0; _i < oldView.length; _i++) {
	    if (oldView[_i] === '#') {
	      var _tempView = '';
	      for (var _j = _i; _j < oldView.length; _j++) {
	        _tempView += oldView[_j];
	        if (oldView[_j + 1] === '/') {
	          break;
	        }
	      }
	...
	var addHandlers = function addHandlers() {
	  ...
	  $(document).on('click', '.portfolioImage', function () {
	    ...
	    var modalImage = [];
	    for (var image in store.portfolioImages) {
	      if (store.portfolioImages[image].id === this.id) {
	        store.currentImage = store.portfolioImages[image];
	        if (store.user !== undefined) {
	          $('#deleteImage').show();
	          var content = '<h2>' + 'Title: ' + '<input type="text" name="image[title]" value="' + store.portfolioImages[image].title + '" class="input-field">' + '</h2>' + '<br>' + '<img src="' + store.portfolioImages[image].link + '" class="modalImage">' + '</img>' + '<br>' + 'Image Link: ' + '<input type="text" name="image[link]" value="' + store.portfolioImages[image].link + '" class="text-box">' + '<br>' + 'Category: ' + '<input type="text" name="image[category]" value="' + store.portfolioImages[image].category + '" class="input-field">' + '<hr>' + 'Description: <br>' + '<textarea type="text" name="image[description]" class="text-box">' + store.portfolioImages[image].description + '</textarea>' + '<input type="hidden" value="' + store.portfolioImages[image].id + '" name="image[id]" />' + '<button type="submit" class="btn btn-xs btn-primary">Save</button>';
	          $('#imageDeailsModalBody').empty();
	          $('#changeImageForm').empty();
	          $('#changeImageForm').append(content);
	        } else {
	          $('#deleteImage').hide();
	          var _content = '<div class="modal-body">' + '<h2>' + store.portfolioImages[image].title + '</h2>' + '<br>' + '<img src="' + store.portfolioImages[image].link + '" class="modalImage"></img>' + '<hr>' + '<p class="imageDescription">' + store.portfolioImages[image].description + '</p>' + '</div>';
	          $('#changeImageForm').empty();
	          $('#imageDeailsModalBody').empty();
	          $('#imageDeailsModalBody').append(_content);
	        }
	      }
	    }
	  });
...
/***/ function(module, exports, __webpack_require__) {
	...
	var createNewImage = function createNewImage(data) {
	  return $.ajax({
	...
	var loadImages = function loadImages(data) {
	  return $.ajax({
	    url: config.host + '/images',
	    method: 'GET'
	  });
	...
	var editImage = function editImage(data) {
	  return $.ajax({
...
/***/ function(module, exports, __webpack_require__) {
	...
	var newImageSuccess = function newImageSuccess(data) {
	  store.portfolioImages.push(data.image);
	  viewUi.displayImages();
	  $('#newImageModal').modal('hide');
	};

	var getImagesSuccess = function getImagesSuccess(data) {
	...
	var editImageSuccess = function editImageSuccess(data) {
	  var images = store.portfolioImages;
	  for (var image in images) {
	...
	module.exports = {
	  failure: failure,
	  success: success,
	  newImageSuccess: newImageSuccess,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var doSomething = function doSomething(event) {
	  var data = getFormFields(this);
	  event.preventDefault();
	...
	var displayImages = function displayImages() {
	  ...
	  for (var image in images) {
	    ...
	    if (images[image].category === filter || filter === 'none') {
	      var html = '<img src="' + images[image].link + '" alt="' + images[image].title + '" class="portfolioImage" id="' + images[image].id + '">' + '</img>';
	      content += html;
	    }
	  }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onNewImage = function onNewImage(event) {
	  event.preventDefault();
	  var data = getFormFields(this);
	  var link = data.image.link;
	  var success = false;
	  for (var i = 0; i < link.length; i++) {
	    if (link[i] === '.') {
	      ...
	      if (ext === 'png' || ext === 'jpg') {
	        success = true;
	        api.createNewImage(data).then(ui.newImageSuccess(data)).catch(ui.failure);
	        break;
	      }
	    }
	  }
	...
	var saveEditedImage = function saveEditedImage(event) {
	  var data = getFormFields(this);
	  event.preventDefault();
	  api.editImage(data).then(ui.editImageSuccess(data)).catch(ui.failure);
	};

	var deleteSelectedImage = function deleteSelectedImage() {
	  event.preventDefault();
	  api.deleteImage().then(ui.deleteImageSuccess).catch(ui.failure);
	};
